...
R016 挡的是一类静默失效:结构被删或改名,而清单还承诺着。
`Equipment` 曾经声明了却零实例、那条派生规则从未触发,就是这个形态 ——
校验不报,读元模型的人以为设备视角已经建好。现在这种分离会报错。
...
真不真通。CQ16 就曾经这样:五个依赖全有实例、R016 放行,而 35 台设备里只有 9 台
走得通 —— 因为 `detected_at` 是检出点,只覆盖量测与测试工序。`--probe` 会把这个
比例直接打出来。清单未覆盖(`3`)时,输出里的"分数不足的候选"若明显对得上,
补 `match_terms` 而不是新增条目。
...
`changesets/pending/20260826-euv-litho.yaml` 是一份可直接试跑的示例提案
（其 web URL 为占位，正式运行须填真实可验证链接）。

## 当前规模

本文档不记录具体数量。规模随每次更新变化，抄一份到文档里就多一个维护点，
而漂了不会有任何报错——曾经写死的"204 条记录"在合并一条实例后就成了错的。
...
- **`economic_hooks` 只有定性传导，没有量化系数**。每个实体都填了
  ...
  和工艺知识同等的地位
- **设备维度只到机台粒度，不到腔体与班别**。35 台设备覆盖全部工序，
...
- **`human` 来源零出现**，全库无人工录入条目。`web` 来源已由
  kb-refresh 首次实跑产出（2026-08-27，`kb.ap.delamination_triage`），
